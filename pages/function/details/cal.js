...
Page({
  ...
  addNew: function (e) {
    ...
    var gradeNew = this.data.grade.concat({ id: length, course: null, creditFocus: false, grade: null, gradeFocus: false, credit: null, });
    // 向grade最后插入一个新元素
  ...
  edit() {
    if (this.data.display) { var editText = '编辑' } else { var editText = '完成' };
    this.setData({ display: !this.data.display, editText: editText })
  },
  sort(e) {
    console.log(e);
  },
  remove(e) {
    console.log(e);
    let currentID = e.target.id[e.target.id.length - 1], grade = this.data.grade;
  ...
  calculate() {
    var that = this, courseNumber = this.data.grade.length;
    // 明确指针避免setData报错// 获得课程数
    console.log("课程数是" + courseNumber);
    var totalCredit = 0, totalGradeCal = 0, flunkingCredit = 0, flunkingGradeCal = 0;
    for (let i = 0; i < courseNumber; i++) {
      let grade = this.data.grade[i].grade, credit = this.data.grade[i].credit;
      if (grade != 0 && grade && credit && credit != 0) {
        // 判断grade和credit是否均有值
        if (grade < 60) {
          flunkingCredit = credit + flunkingCredit;
          if (grade > 50) { flunkingGradeCal = (grade - 50) / 10 * credit + flunkingGradeCal }
