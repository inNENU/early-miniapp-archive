...
Page({
    ...
    page: [
      ...
      { tag: 'p', head: '当前版本已实现的特性：', text: '独家主题模式，已经初步构建wechat与iOS两种主题，第一次进入小程序会自动根据系统来加载对应的主题；iOS主题实现99%界面仿真，已实现iOSnav特性动画；夜间模式已加入，拥有其开启开关和自动开启开关，可以在设置中自行选择开始时间与结束时间；东师指南所有子界面实现动态加载，拉取指定的json文件完成界面构建；所有界面文件支持Wifi下下载并本地缓存，界面文件更新时具有更新提示；' },
      {
        tag: 'list', head: '日志如下：', content: [
          ...
          { text: '亮度', slider: 'brightness', min: 0, max: 100, calue: 50, key: 'brightness' }]
      },
  ...
  brightness(e) {
    ...
    if (e.type == 'tap') { content.visible = !content.visible; this.setData({ page: page }) }
    else if (e.type == 'changing') { content.value = e.detail.value; this.setData({ page: page }) }
    else if (e.type == 'change') {
      content.value = e.detail.value;
      this.setData({ page: page });
      wx.setStorageSync(content.key, e.detail.value)
    };
