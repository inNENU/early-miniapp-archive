...
$register('map', {
  ...
  onNavigate() {
    console.log('将要跳转Map');
    markerSet();
    globalData.marker = this.getMarker();
  },
  ...
  onLoad() {
    ...
    if (globalData.marker) {
      ({ isBenbu, markers } = globalData.marker);
      delete globalData.marker;
    } else {
      markerSet();
  ...
  showList() {
    if (this.data.list) {
      ...
      this.setData({
        list: !this.data.list,
        closeTop: globalData.info.statusBarHeight + 5.5
      });
  },
