...
|action|否|String|左上角按钮触发的函数名称，界面为首页时设置ture(其值为true时会取消返回按钮)|不填,执行返回|
|grey|否|Boolean|属性值为true时会使用灰色背景|false，设置为白色背景|

##### **h3参数：**
|参数|是否必填|值类型|内容|默认内容|
|-|:-:|:-:|-|-|
|text|是|String|展示页的大标题文字||
|style|否|String|添加css样式以改变标题的风格(字体粗细、大小、对齐、边距等)||

##### **p参数：**
|参数|是否必填|值类型|内容|默认内容|
|-|:-:|:-:|-|-|
|head|否|String或Boolean|填入段落的的标题，也可设置为true在段落前留空占位||
|text|是|String|段落的文字||
|src|否|String|图片的本地路径或在线网址，在段落文字底部展示所选图片||
|res|否|String|图片在服务器上的网址(在需要高清图片的时候使用此选项)||
|imgmode|否|String|图片的显示模式，不填为appData设定的默认值||
