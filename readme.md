...
| 参数 |是否必填| 值类型 | 内容 | 默认内容 |
|- | :-: |:-: | - | - |
|text| 是|String | 填入标题和导航栏显示的文字||
|desc|否|String |填入标题描述文字，该文字只在特定主题下展示||
|action|否|String |填入左上角按钮触发的函数名称，界面为首页时设置ture(其值为true时会取消返回按钮)|不填,功能是返回|
|grey|否|Boolean|设置属性值为true时会用灰色背景取代默认的白色背景|false|

- action(可选)：接受一个String值，填入左上角按钮触发的函数名称，默认不填为返回，界面为首页时设置ture(其值为true时会取消返回按钮)；
##### **h3参数：**
