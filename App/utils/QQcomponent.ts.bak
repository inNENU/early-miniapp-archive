 ...
 * @LastEditors: Mr.Hope
 * @LastEditTime: 2019-08-09 11:06:54
 * @Description: 组件函数库
 */
...
const documentHandler = (res: NormalEvent) => {
  ...
  if (['doc', 'ppt', 'xls', 'pdf'].includes(doctype)) {
    ...
    // 检测到图片，开始图片浏览
  } else if (['jpg', 'png', 'gif'].includes(doctype)) wx.previewImage({ urls: [url] });
};
