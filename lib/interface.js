 ...
 * @LastEditTime: 2019-04-11 23:42:45
 */
...
const logger = wx.getLogManager({ level: 1 }),
   ...
   * @param {callback} [failFunc = () => null] 失败回调函数
   * @param {callback} [failFunc = () => null] 状态码错误为回调函数
   *
   * @returns {void}
   */
  downLoad = (path, callbackFunc, failFunc = () => null, errorFunc = () => null) => {
    const progress = wx.downloadFile({
      url: `https://mp.nenuyouth.com/${path}`,
      success: res => {
        if (res.statusCode === 200) callbackFunc(res.tempFilePath);
        else {
          tip('服务器出现问题，请稍后重试');
          errorFunc(res.statusCode);
