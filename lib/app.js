...
const fileManager = wx.getFileSystemManager(),
  ...
  resDownload = list => {
    ...
    list.forEach(name => {
      // 下载zip包
      wx.downloadFile({
        url: `https://mp.nenuyouth.com/page/${name}.zip`,
        success: res => {
  ...
  noticeCheck = () => {
    ...
    // 获取通知列表
    wx.request({
      url: 'https://mp.nenuyouth.com/notice.json',
      ...
      success: res => {
        ...
        if (res.statusCode === 200) {
          ...
          logger.warn('noticeList error', 'Address Error');// 调试
        }
      },
      ...
      fail: () => {
        ...
        wx.reportMonitor('24', 1);
        logger.warn('noticeList error', 'Net Error');// 调试
      }
    });
  },
   ...
   * @returns {void}
   */
  appUpdate = (forceUpdate = true, reset = false) => {
    const updateManager = wx.getUpdateManager();
    let version, configForceUpdate, configReset;
    ...
    updateManager.onCheckForUpdate(status => {
      ...
      if (status.hasUpdate) {
        ...
        console.warn('发现小程序新版本');// 调试

        // 请求配置文件
        wx.request({
          url: 'https://mp.nenuyouth.com/config.json',
          success: res => {
            console.log(res); // 调试

            // 获取成功，覆盖默认参数
            if (res.statusCode === 200) ({ configForceUpdate, configReset } = res.data);
    ...
    updateManager.onUpdateReady(() => {
      ...
      // 更新下载就绪，提示用户重新启动
      if (configForceUpdate && forceUpdate) wx.showModal({
        title: '已找到新版本',
        content: `版本${version}已下载，请重启应用更新。${(configReset || reset ? '该版本会初始化小程序。' : '')}`,
