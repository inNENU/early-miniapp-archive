...
const appInit = () => {
  ...
  // 写入预设数据
  for (let data in appOption) if (data != "theme") wx.setStorageSync(data, appOption[data]);
...
const nightmode = () => {
  ...
  let nightModeCondition = wx.getStorageSync("nightmode"),
    ...
    nightmodeEndTime = Number(nmEnd[0]) * 100 + Number(nmEnd[1]), currentNightModeStatus;
 ...
 * @returns {void}
 */
const appUpdate = (forceUpdate = true, reset = false) => {
  ...
  updateManager.onUpdateFailed(() => {
    ...
    console.warn("Update failure"), wx.reportMonitor("23", 1), logger.warn("Upate App error because of Net Error");// 调试
