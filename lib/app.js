...
const hint = res => { // 取消初始化提示
  listener[res] = true;
  if (listener.page && listener.function) wx.hideLoading();
};

const resDownload = name => { // 下载name对应资源
      ...
      if (res.statusCode === 200) {
        ...
        fileManager.unzip({
          zipFilePath: `${userPath}/${name}Zip`, targetPath: userPath, success: () => {
            ...
            wx.setStorageSync(`${name}Download`, true), hint(name);
