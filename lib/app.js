...
const resDownload = path => {
  wx.downloadFile({
    url: `https://nenuyouth.com/Res/${path}.zip`,
    success: res => {
      console.log(`${path} statusCode is ${res.statusCode}`);
      if (res.statusCode === 200) {
        fileManager.saveFileSync(res.tempFilePath, `${userPath}/${path}Zip`);
        console.log("save success");
        fileManager.unzip({
          zipFilePath: `${userPath}/${path}Zip`, targetPath: userPath, success: () => {
            console.log("unzip sucess");// 调试
            fileManager.unlinkSync(`${userPath}/${path}Zip`);
            wx.setStorageSync(`${path}Download`, true);
            console.log("delete sucess");// 调试
          },
          fail: msg2 => console.error(`unzip ${path} fail:`, msg2)
        });
      }
    },
    fail: msg1 => console.error(`download ${path} fail:`, msg1)
  });
...
const appInit = data => {
  ...
  wx.setStorageSync("funcNotify", true), wx.setStorageSync("resNotify", true);
  wx.setStorageSync("localFuncTime", Math.round(new Date() / 1000));
  wx.setStorageSync("localResTime", Math.round(new Date() / 1000));
