 ...
 * 获取文件夹与路径名称
 *
 * @param {string} aim 页面名称
 * @returns {*}
 */
const resolveAim = aim => {
  let { length } = aim;

  while (!isNaN(aim.charAt(length))) length--;
...
/**
 * 设置界面，在onNavigate时调用，预处理page数据写入存储
 *
 * @param {*} page page数组
 * @param {*} globalData 全局数据
 * @param {*} option 页面传参
 ...
 * @returns {void}
 */
const resolvePage = (globalData, option, page) => {
  console.info('将要跳转：', option); // 控制台输出参数
  if (page) globalData.page.data = disposePage(page, globalData, option);
  else {
    const aim = option.query ? option.query.aim : option.aim;

    const { path } = resolveAim(aim);

    $file.getJson(`page/${path}`, pageData => {
      if (pageData) globalData.page.data = disposePage(pageData, globalData, option);
      else {
    ...
    console.info('globalData', globalData2);
  }
...
const setPage = ({ a: globalData, res: option, ctx, set }, page, preload = true) => {
  ...
  if (globalData.page.aim === option.aim) {
    ...
    const pageData = page ? page : ctx.data.page;

    // 设置页面数据
    ctx.setData({
      T: globalData.T,
      nm: globalData.nm,
      page: set ? pageData : disposePage(pageData, globalData, option)
    });
  }
};
