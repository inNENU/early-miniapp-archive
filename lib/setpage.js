 ...
 * @returns {*} 处理之后的page
 */
const disposePage = (page, globalData, option) => {
  if (page)  // 如果page参数传入
    if (page[0].tag == "head") {
      ...
      if (option && !page[0].action) {
        "aim" in option ? page[0].aim = option.aim : page[0].aim = page[0].title; // 设置界面名称
        if ("From" in option) page[0].leftText = option.From; // 设置页面来源
        if ("depth" in option) page[0].aimDepth = Number(option.depth) + 1; // 设置界面路径深度
        if ("share" in option) page[0].action = "redirect", console.info("redirect"); // 判断是否来自分享，分享页重定向到sharePage
      }
      page.forEach((element, index) => {
        ...
        // 处理图片
        if (element.src) {
          element.res ? page[0].url.push(element.res) : page[0].url.push(element.src), element.res = element.src;
          if (!element.imgMode) element.imgMode = "widthFix";
        }

        // 处理文档
        if (element.docName) {
          let temp = element.docName.split(".")[1];
        ...
        if ("content" in element) element.content.forEach((listElement, listIndex) => {
          ...
          if ("pickerValue" in listElement)
            if (listElement.single) { // 单列选择器
              let pickerValue = wx.getStorageSync(listElement.key);
              listElement.value = listElement.pickerValue[pickerValue], listElement.currentValue = [pickerValue];
            } else { // 多列选择器
              let pickerValues = wx.getStorageSync(listElement.key).split("-");
              listElement.currentValue = [], listElement.value = [];
              pickerValues.forEach((k, l) => {
                listElement.value[l] = listElement.pickerValue[l][Number(k)], listElement.currentValue[l] = Number(k);
              });
            }
 ...
 * @returns {void}
 */
const getOnlinePage = (option, globalData, ctx) => {

  // 确定文件夹名称
  let length = option.aim.length, pageData;
  while (!isNaN(option.aim.charAt(length))) length--;
  let aimName = option.aim.substring(0, length + 1);

  // 尝试从存储中读取界面数据
  try {
    // 尝试从文件中解析页面数据
    pageData = JSON.parse(fileManager.readFileSync(`${userPath}/page/${aimName}/${option.aim}.json`, "utf-8"));
    ...
    // 向服务器请求json
    wx.request({
      url: `https://nenuyouth.com/Res/page/${aimName}/${option.aim}.json`,
      success: res => {
        console.log(res); // 调试

        // 资源获取正常
        if (res.statusCode == 200) {
          ctx.$session.set(`${option.aim}Temp`, disposePage(res.data, globalData, option)); // 页面处理结果写入缓存
          if (!option.share) { // 非分享界面下将页面数据写入存储
            try {
              fileManager.mkdirSync(`${userPath}/page/${aimName}/${option.aim}.json`, true);
            } catch (err) {
              console.log("目录已存在");
            }
            fileManager.writeFileSync(
              `${userPath}/page/${aimName}/${option.aim}.json`,
              JSON.stringify(res.data), "utf8"
            );
          }
 ...
 * @returns {void}
 */
const setOnlinePage = (globalData, option, ctx, preload = true) => {
  ...
  // 需要重新载入界面
    ...
    console.info("onLoad开始，参数是：", option);
    ctx.aim = option.aim;

    // 获取文件夹名称
    let length = option.aim.length;
    while (!isNaN(option.aim.charAt(length))) length--;
    let aimName = option.aim.substring(0, length + 1);
    ...
    wx.request({
      ...
      success: res => {
        ...
        if (res.statusCode == 200) {
          ...
          // 设置界面
          setPage(disposePage(res.data, globalData, option), globalData, option, ctx);
          ...
          if (!option.share) {
            try {
              ...
              console.log(`${option.aim}目录已存在`);
            }
            fileManager.writeFileSync(`${userPath}/page/${aimName}/${option.aim}.json`, JSON.stringify(res.data), "utf8");
          }
...
const picker = (res, ctx) => {
  let position = res.currentTarget.dataset.id.split("-"),
...
const slider = (res, ctx) => {
  let pos = res.currentTarget.dataset.id.split("-"),
    content = ctx.data.page[pos[0]].content[pos[1]], value = res.detail.value;
...
const Switch = (res, ctx) => {
  let pos = res.target.dataset.id.split("-");
...
const documentHandler = res => {
  let { doctype, url } = res.currentTarget.dataset; // 解构赋值
 ...
 */
const phone = (res, ctx) => {
  let Type = res.target.dataset.type, info = ctx.data.page[res.currentTarget.id];
...
const share = (res, ctx) => {
  ...
  if (res.type == "touchstart") {
    ctx.left = touch.pageX - res.currentTarget.offsetLeft, ctx.top = touch.pageY - res.currentTarget.offsetTop;
...
const video = res => {
  ...
  if (res.type == "waiting") wx.showToast({ title: "缓冲中...", icon: "none" });// 视频缓冲时提示用户等待
...
const changeNav = (option, ctx) => {
  let pageHead = ctx.data.page[0], titleDisplay, borderDisplay, shadow;

  // 判断情况并赋值
  if (option.scrollTop <= 1) titleDisplay = borderDisplay = shadow = false;
  else if (option.scrollTop <= 42) titleDisplay = borderDisplay = false, shadow = true;
  else if (option.scrollTop >= 53) titleDisplay = borderDisplay = shadow = true;
  else titleDisplay = shadow = true, borderDisplay = false;
