...
const logger = wx.getLogManager({ level: 1 }),
  ...
  getOnlinePage = (option, globalData, ctx) => {
    ...
    try {
      // 尝试从文件中解析页面数据
      pageData = $file.readJson(`page/${folder}/${option.aim}.json`);
      ...
      // 向服务器请求json
      wx.request({
        url: `https://mp.nenuyouth.com/page/${folder}/${option.aim}.json`,
        success: res => {
          console.log(res); // 调试
          ...
          if (res.statusCode === 200) {
            // 页面处理结果写入缓存
            ctx.$session.set(`${option.aim}Temp`, disposePage(res.data, globalData, option));

            // 非分享界面下将页面数据写入文件存储
            if (!option.share) {
              $file.makeDir(`page/${folder}`);
              $file.writeJson(`page/${folder}`, `${option.aim}.json`, res.data);
            }
  ...
  setOnlinePage = (globalData, option, ctx, preload = true) => {
    ...
    if (ctx.aim === option.aim) {
      ...
      const aimName = option.aim.substring(0, length + 1),
        page = $file.readJson(`page/${aimName}/${option.aim}.json`);

      // 如果本地存储中含有page直接处理
      if (page) {
        setPage(disposePage(page, globalData, option), globalData, option, ctx);
        ...
        wx.request({
          ...
          success: res => {
            ...
            if (res.statusCode === 200) {
              ...
              // 非分享界面下将页面数据写入存储
              if (!option.share) {
                $file.makeDir(`page/${aimName}`);
                $file.writeJson(`page/${aimName}`, `${option.aim}.json`, res.data);
              }
