...
const fileManager = wx.getFileSystemManager(),
  ...
  getOnlinePage = (option, globalData, ctx) => {
    ...
    try {
      ...
      // 向服务器请求json
      wx.request({
        url: `https://mp.nenuyouth.com/page/${aimName}/${option.aim}.json`,
        success: res => {
          console.log(res); // 调试
  ...
  share = (res, ctx) => {
    ...
    // 取消显示菜单，并执行对应逻辑
      ...
      if (res.target.dataset.object === 'download') {
        ...
        wx.downloadFile({
          url: `https://mp.nenuyouth.com/img/share/${ctx.data.page[0].aim}.jpg`,
          success: res1 => {
            console.log(res1);
  ...
  video = res => {
    ...
    // 提示用户播放错误
      ...
      wx.showToast({ title: '视频加载出错', icon: 'none', duration: 2000 });
    ...
    if (globalData.nm && grey)
      ...
      switch (globalData.T) {
        ...
        case 'NENU': color = ['#f0f0f0', '#f0f0f0', '#f0f0f0'];
          break;
        case 'iOS':
        default: color = ['#f4f4f4', '#efeef4', '#efeef4'];
      }
    else
      switch (globalData.T) {
        case 'Andriod': color = ['#f8f8f8', '#f8f8f8', '#f8f8f8'];
