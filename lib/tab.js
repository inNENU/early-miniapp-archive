...
const checkResUpdate = (name, dataUsage) => {
  ...
  const currentTime = Math.round(new Date() / 1000);// 读取当前和上次更新时间
 ...
 * @returns {void}
 */
const refreshPage = (name, ctx, globalData) => {
  const test = wx.getStorageSync('test');

  // 开启测试时刷新页面
  if (test) $wx.request(`config/${name}Test`, data => {
    wx.setStorageSync(name, data);
    $page.Set({ a: globalData, res: { aim: name }, ctx }, data);
  });
  // 正常加载逻辑
...
module.exports = { update: checkResUpdate, tabBarChanger, refresh: refreshPage, markerSet, resDownload };
