 ...
 * wx.request包装
 *
 * @param {*} path 请求路径(Res文件夹下)
 * @param {*} callbackFunc 回调函数
 * @returns {void}
 */
const request = (path, callbackFunc) => {
  wx.request({
    ...
    fail: failMsg => {
      console.log(failMsg), wx.reportMonitor("4", 1);
    }
  });
};
...
const checkResUpdate = (name, dataUsage) => {
  let notify = wx.getStorageSync(`${name}ResNotify`), // 资源提醒
    localVersion = $file.readFile(`${name}Version.json`), // 读取本地Version文件
 ...
 * @returns {void}
 */
const tabBarChanger = nightmode => {
  let color = nightmode ? ["#000000", "white"] : ["#ffffff", "black"];
...
const initMarker = markers => {
  markers.forEach(marker => {
    ...
    delete marker.name, delete marker.detail;
